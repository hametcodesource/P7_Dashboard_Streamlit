...
def general(df_analyse):
    ...
    ** Les variables sont classes de haut en bas par ordre d'importance dans l'interpretation.** 
    Ici la variable APP_EXT_SOURCE_2_EXT_SOURCE_3_DAYS_BIRTH est donc celle qui a le plus d'importance
    ...
    **Enfin, l'impact sur la prediction est representé par le côté gauche ou droit.
     ...
     Ainsi pour la Variable APP_EXT_SOURCE_2_EXT_SOURCE_3_DAYS_BIRTH, des grandes valeurs de celle-ci vont faire baisser le score de prediction, 
     on aura plus de chances d'accepter le crédit 
    
    
    """)
    st.image('shap.png', width=600)